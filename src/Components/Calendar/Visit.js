    ...
    return (
        <Card className={visit.petName === null? "alert-primary" : "alert-danger"} style={{ width: '12rem', maxHeight: '7rem', marginBottom: 10}}>
            <Card.Body>
                ...
                <Card.Subtitle className="mb-2 text-muted">{new Date(visit.beginTime).getHours()}.{new Date(visit.beginTime).getMinutes()} - {new Date(visit.endTime).getHours()}.{new Date(visit.endTime).getMinutes()}</Card.Subtitle>
                <Card.Text>
